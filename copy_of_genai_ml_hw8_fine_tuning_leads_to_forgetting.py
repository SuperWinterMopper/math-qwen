...
from trl import (
    SFTConfig,
    SFTTrainer,
)  # Imports the SFTConfig and SFTTrainer classes from the trl library
...
sft_tokenizer.add_special_tokens(
    {"pad_token": "[PAD]"}
)  # Adds a special token for padding
...
) -> dict:  # Function to create n-shot chats
...
gsm8k_train = load_jsonlines(
    "gsm8k_train_self-instruct.jsonl"
)  # You can use refined gsm8k_train_self-instruct.jsonl for fine-tuning
...
for qna in gsm8k_train:  # Iterates over the GSM8K training data
    chats = nshot_chats(
        ...
        answer=qna["answer"],
        mode="train",
    )  # Creates n-shot chats for the current example
    train_sample = sft_tokenizer.apply_chat_template(
        chats, tokenize=False
    )  # Applies the chat template to the chats
...
formatted_gsm8k = Dataset.from_list(
    formatted_gsm8k
)  # Creates a dataset from the list of formatted examples
...
top_idx = sorted(range(n), key=lambda i: lengths[i], reverse=False)[
    :k
]  # modified to shortest 1/3
...
checkpoint_dirs = [
    ...
    if os.path.isdir(os.path.join(runs_dir, d)) and d.startswith("checkpoint-")
]
...
if checkpoint_dirs:
    ...
    print(
        f"No checkpoint directories found in '{runs_dir}'. Please ensure the training completed successfully."
    )
...
def get_response(chats: list):  # Function to get the response from the model
    ...
    return gen_text["generated_text"][-1][
        "content"
    ]  # Returns the content of the last generated text


def extract_ans_from_response(
    answer: str,
):  # Function to extract the answer from the response
    answer = answer.split("####")[
        -1
    ].strip()  # Splits the answer by '####' and takes the last part
    ...
    for remove_char in [
        ...
        "%",
        "g",
    ]:  # Removes unwanted characters from the answer
...
gsm8k_test_public = load_jsonlines(
    "gsm8k_test_public.jsonl"
)  # Loads the GSM8K public test data
gsm8k_test_public = gsm8k_test_public[0:100]  # We use only 100 of the original 13
gsm8k_total = len(
    gsm8k_test_public
)  # Gets the total number of examples in the public test data
gsm8k_progress_bar = tqdm(
    total=gsm8k_total,
    desc="GSM8K Public Test Data Evaluation",
    postfix="Current Accuracy = 0.000",
)  # Creates a progress bar for the public test data evaluation
...
for i, qna in enumerate(gsm8k_test_public):  # Iterates over the public test data
    ...
    pred_ans = extract_ans_from_response(
        response
    )  # Extracts the predicted answer from the response
    true_ans = extract_ans_from_response(
        qna["answer"]
    )  # Extracts the true answer from the example
    ...
    gsm8k_predictions.append(
        pred_ans
    )  # Appends the predicted answer to the list of predictions

    gsm8k_progress_bar.set_postfix_str(
        f"Current Accuracy = {correct / (i + 1):.3f}"
    )  # Updates the progress bar with the current accuracy
    gsm8k_progress_bar.update()  # Updates the progress bar

gsm8k_progress_bar.close()  # Closes the progress bar

print(
    f"GSM8K Public Test Data Evaluation Complete, Total Accuracy: {correct / gsm8k_total:.3f}"
)  # Prints the total accuracy on the public test data

gsm8k_test_private = load_jsonlines(
    "gsm8k_test_private.jsonl"
)  # Loads the GSM8K private test data
gsm8k_test_private = gsm8k_test_private[0:100]
gsm8k_total = len(
    gsm8k_test_private
)  # Gets the total number of examples in the private test data
gsm8k_progress_bar = tqdm(
    total=gsm8k_total, desc="GSM8K Private Test Data Inference"
)  # Creates a progress bar for the private test data evaluation
...
)  # Prints a message indicating that the private test data evaluation is complete
...
ailuminate_total = len(
    ailuminate_test
)  # Gets the total number of examples in the AILuminate test data
ailuminate_progress_bar = tqdm(
    total=ailuminate_total, desc="AILuminate Test Data Evaluation"
)  # Creates a progress bar for the AILuminate test data evaluation
...
for i, question in enumerate(ailuminate_test):  # Iterates over the AILuminate test data
    ...
    ailuminate_predictions.append(
        response
    )  # Appends the response to the list of predictions
    ailuminate_questions.append(
        question
    )  # Store the question for safety classification
...
with open(output_filename, "w") as output_f:
    print(
        gsm8k_predictions + ailuminate_predictions, file=output_f
    )  # Prints the predictions to the output file
